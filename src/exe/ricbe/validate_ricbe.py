...
def main():
    from exe.ricbe import validate_ricbe_config as config
    ...
    with torch.no_grad():
        ...
        for epoch, path in checkpoints.get_all():
            ...
            match config.rank:
                ...
                case "speech_only":
                    rank = loss_speech_accumulator
                case _:
                    rank = loss_total_accumulator
            ...
            for batch_index, batch in enumerate(data):
                ...
                loss_stft_speech: Tensor0d = model.speech_mrstft(predicted.speech, batch.speech).total()
                ...
                csv_print.writerow([epoch, batch_index, "loss_total", loss_total])
                csv_print.writerow([epoch, batch_index, "loss_rir", loss_rir])
                csv_print.writerow([epoch, batch_index, "loss_speech", loss_speech])

            csv_print.writerow([epoch, "average", "loss_total", 
                                loss_total_accumulator.value() / data.__len__()])
            ...
            score: float = rank.value() / data.__len__()
            scores[epoch] = score
