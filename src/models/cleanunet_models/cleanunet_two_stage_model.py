...
from models.cleanunet_models.networks.cleanunet_two_stage_network import CleanUNetTwoStageNetwork
from trainers.trainable import Trainable


class CleanUNetTwoStageModel(Trainable):
    def __init__(self, device: torch.device) -> None:
        super().__init__()
        self.device = device

        self.module = CleanUNetTwoStageNetwork().to(device)
    ...
    def _predict(self,
        ...
        return CleanUNetTwoStageModel.Prediction(Tensor2d(speech.squeeze(1)), 
    ...
    def prepare_train_on(self, 
        ...
        predicted: CleanUNetTwoStageModel.Prediction = self._predict(reverb_batch)
