...
from models.cleanunet_models.networks.cleanunet_fullbe_network import CleanUNetFullbeNetwork
...
class CleanUNetFullbeModel(Trainable):
    def __init__(self, device: torch.device) -> None:
        super().__init__()
        self.device = device

        self.module = CleanUNetFullbeNetwork().to(device)
    ...
    def _predict(self,
        ...
        return CleanUNetFullbeModel.Prediction(Tensor2d(speech.squeeze(1)), 
    ...
    def prepare_train_on(self, 
        ...
        predicted: CleanUNetFullbeModel.Prediction = self._predict(reverb_batch)
